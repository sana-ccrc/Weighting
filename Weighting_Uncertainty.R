...
# Apply a set of weights on bias corrected models (columns of df)
Get_weighted_vec <- function(df, w){
  weighted_vec <- rep(0, nrow(df))
  for(i in 1:ncol(df)){
    weighted_vec <- weighted_vec + w[i]*df[i]
  }
  return(weighted_vec)
}
